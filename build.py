...
<h3>但有一個強而有力的支撐論點</h3>
<div class="callout ok"><b>✓ 臺中市建築管理自治條例第 19 條第 4 款：「曾指定建築線且已核准建築完成之巷道」即屬現有巷道。</b>
福利路 117 巷兩側建物林立、皆為合法建築並領有門牌
（實價登錄可查得 117 巷、118 巷多筆房地交易，屋齡自民國 69 年至 103 年不等），
<b>這些房屋既然能合法起造完成，代表此巷在過去必定已被指定過建築線</b>。<br>
加上第 1、2 款所稱「經道路主管機關認定屬既成道路」、「已納入維護或管理之公眾通行道路」
—— 本巷已開闢逾四十年、鋪設柏油、劃設標線、設有排水溝與門牌，
<b>符合既成道路要件的機會相當高，且此三款均不需取得土地所有權人同意書</b>。<br>
<b>換言之：雖然屋主沒有持分，但本案很可能不需要 375 共有人的同意即可指定建築線。</b>
需要同意書的是第 6 款「私設通路」的情形，本案應可循第 1、2、4 款認定。</div>

<div class="callout warn"><b>⚠ 但這個「很可能」必須變成「確定」。</b>
在未取得建築線指定書面之前，<b>本案存在「能否建築」的根本性不確定</b>，
而這正是買方出價最保守的原因。<br>
<b>強烈建議：委託後的第一件事，就是申請指定建築線（規費 500 元）。</b>
結果會直接決定三件事 ——
① 是否需退讓、退讓多少（影響 4.2–8.5 坪）；
② 建築線認定依據為何款（影響買方信心）；
③ 開價應落在區間的上緣或下緣。
<b>這 500 元是本案最划算的一筆支出，沒有之一。</b></div>
...
<span>張現傑</span>
<span>製表日期：2026.09.22（第 10 版・通行權查證）</span>
</div></footer>
</body></html>"""
